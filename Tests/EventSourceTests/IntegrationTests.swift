...
import XCTest
    ...
    struct IntegrationTests {
        ...
        @Test("Decode token chunks from SSE stream")
        func testDecodingTokenChunks() async throws {
            // Define the test URL
            let url = URL(string: "https://api.example.com/completions")!
            ...
            // Verify the final result
            XCTAssertEqual(completedText, "Hello, world!")
            XCTAssertEqual(receivedChunks.count, 3)
            XCTAssertEqual(receivedChunks[0], TokenChunk(text: "Hello, ", isComplete: false))
            XCTAssertEqual(receivedChunks[1], TokenChunk(text: "world", isComplete: false))
            XCTAssertEqual(receivedChunks[2], TokenChunk(text: "!", isComplete: true))
        }
@Test("Full API integration with POST request")
        func testFullAPIIntegration() async throws {
            // Define the test URL
            let url = URL(string: "https://api.example.com/completions")!
            ...
            await MockURLProtocol.setHandler { request in
                ...
                if let bodyData = request.httpBody {
                    let bodyString = String(data: bodyData, encoding: .utf8)
                    XCTAssertTrue(bodyString?.contains("prompt") == true)
                    XCTAssertTrue(bodyString?.contains("max_tokens") == true)
                }
            ...
            // Verify the final result
            XCTAssertEqual(completedText, "Hello there!")
        }

        @Test("Use EventSource for token streaming")
        func testEventSourceForTokenStreaming() async throws {
            // Define the test URL
            let url = URL(string: "https://api.example.com/completions")!
            ...
            await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
                ...
                eventSource.onMessage = { event in
                    do {
                        ...
                        XCTFail("Failed to decode event: \(error)")
                        Task {
                            await eventSource.close()
                            continuation.resume()
                        }
            ...
            // Verify the final result
            let getText = await responseState.getText()
            let eventCount = await responseState.getEventCount()
            let isComplete = await responseState.isCompleted()

            XCTAssertEqual(getText, "Hello there!")
        ...
        func testEventSourceReconnection() async throws {
            ...
            // Check the results
            let connectionCount = await counter.getCount()
            XCTAssertGreaterThanOrEqual(connectionCount, 2, "Should have attempted at least 2 connections")

            let openCount = await tracker.getOpenCount()
            XCTAssertGreaterThanOrEqual(openCount, 1, "Should have opened at least once")

            let errorCount = await tracker.getErrorCount()
            XCTAssertGreaterThanOrEqual(errorCount, 1, "Should have encountered at least one error")

            let events = await tracker.getEvents()
            XCTAssertGreaterThanOrEqual(events.count, 1, "Should have received at least one event")

            if let firstEvent = events.first {
                XCTAssertEqual(firstEvent.id, "123")
                XCTAssertEqual(firstEvent.data, "first message")
            }

            // Check if Last-Event-ID was sent on reconnection
            let lastEventID = await counter.getLastEventID()
            XCTAssertEqual(lastEventID, "123", "Should reconnect with the last event ID")
        ...
        struct MockURLProtocolTests {
            ...
            @Test("Parse SSE Events from URLSession bytes")
            func testParseEventsFromURLSession() async throws {
                // Define the URL and expected event data
                let url = URL(string: "https://example.com/events")!
                let sseData = """
            ...
            @Test("Complex SSE event with all fields")
            func testComplexSSEEvent() async throws {
                // Define the URL and complex event data
                let url = URL(string: "https://example.com/events")!
                let sseData = """
                ...
                XCTAssertEqual(event?.id, "1234")
                XCTAssertEqual(event?.event, "update")
                XCTAssertEqual(event?.data, "{\"name\":\"test\",\"value\":42}")
                XCTAssertEqual(event?.retry, 3000)
            }

            @Test("Chunked delivery simulation with URLSession")
            func testChunkedDeliveryWithURLSession() async throws {
                // Define the URL
                let url = URL(string: "https://example.com/events")!
            ...
            @Test("Content-Type validation with EventSource")
            func testContentTypeValidation() async throws {
                // Define the URL
                let url = URL(string: "https://example.com/events")!
                ...
                if case .invalidContentType = receivedError {
                    ...
                    XCTFail("Expected invalidContentType error but got \(String(describing: receivedError))")
                }
            ...
            @Test("Real-world streaming example")
            func testRealWorldStreamingExample() async throws {
                // Define the URL
                let url = URL(string: "https://api.example.com/stream")!
        ...
        struct MockURLProtocolStreamTests {
            ...
            @Test("Simulated Chunked Delivery Test")
            func testSimulatedChunkedDelivery() async throws {
                // Define the URL
                let url = URL(string: "https://example.com/events")!
                ...
                await MockURLProtocol.setHandler { request in
                    ...
                    return (response, Data(eventChunks.joined().utf8))
                }
            ...
            func testLargeEventDelivery() async throws {
                ...
                // Verify the event
                XCTAssertNotNil(event)
                XCTAssertEqual(event?.data, largePayload)
            }

            @Test("Simulating network conditions with ChunkedAsyncBytes")
            func testNetworkConditionsWithChunked() async throws {
                // Create event data - we'll break this up manually
                let sseData = "data: event with spaces\n\ndata: another\ndata: line\n\n"
                ...
                // Verify the events
                XCTAssertEqual(events.count, 2)
                XCTAssertEqual(events[0].data, "event with spaces")
                XCTAssertEqual(events[1].data, "another\nline")
            }
        }
    }
#endif
